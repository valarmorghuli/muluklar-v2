...
// Traverse ALL (children + _children)
...
function initTree(data) {
  ...
  update(root);
  fitToView(60, 400, { onlyZoomOut: true });
}
...
// ---------------- UI wiring ----------------
function attachUI(){
  const s=$$("#search"), r=$$("#resetFilter"), e=$$("#expandAll"), c=$$("#collapseAll"), h=$$("#homeBtn");

  // live highlight — sadece ilk eşleşmenin yolu açılır
  s.addEventListener("input", () => {
    ...
    if (!q){
      ...
      fitToView(60, 250, { onlyZoomOut: true });
      return;
    }
  ...
  // Tümünü Aç/Kapat
  e.addEventListener("click", () => { expandAll(root); update(root); fitToView(60, 300, { onlyZoomOut: true }); });
  c.addEventListener("click", () => { (root.children || []).forEach(collapseAll); update(root); fitToView(60, 300, { onlyZoomOut: true }); });

  // Merkezle
  h?.addEventListener("click", () => { fitToView(60, 300, { onlyZoomOut: true }); });
}
